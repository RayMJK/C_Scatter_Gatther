...
int createWritePacket(file_entry *entry, char *data, char *output) {
   char recvPacket[SG_DATA_PACKET_SIZE];
   size_t pktlen, rpktlen;
   SG_Node_ID loc, rem;
   SG_Block_ID blkid;
   ...
   if ( (ret = serialize_sg_packet( sgLocalNodeId, // Local ID
                           ...
                           SG_SEQNO_UNKNOWN,    // Sender sequence number
                           SG_SEQNO_UNKNOWN,  // Receiver sequence number
                           data, output, &pktlen)) != SG_PACKT_OK ) {
         return( -1 );
   }
...
unsigned char get_uchar_uint64_t(uint64_t origin, int pos){
   ...
   return (unsigned char)(origin % 256);
}
...
int sgread(SgFHandle fh, char *buf, size_t len) {
      ...
      char *reqData = data + (entry->off % SG_BLOCK_SIZE);
...
int sgwrite(SgFHandle fh, char *buf, size_t len) {
   ...
   char initPacket[initPacketSize];
   char recvPacket[recvPacketSize];
   if (createWritePacket(entry, buf, initPacket) == -1) {
      return( -1);
   }
   ...
   entry->off = entry->off + SG_BLOCK_SIZE;
   entry->size = entry->size + SG_BLOCK_SIZE;
   return( len );
}
...
int returnmsg = 0;
   ...
   if(data != NULL){ // if there are data
